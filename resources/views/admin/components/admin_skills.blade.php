<section class="resume-section" id="skills">
    <div class="resume-section-content">
        <h2 class="mb-5">Skills</h2>
        <div class="subheading mb-3">Programming Languages & Tools</div>
        <ul class="list-inline dev-icons">
            <li class="list-inline-item"><i class="fab fa-golang"></i></li>
            <li class="list-inline-item"><i class="fab fa-laravel"></i></li>
            <li class="list-inline-item"><i class="fab brands fa-docker"></i></li>
            <li class="list-inline-item"><i class="fab brands fa-aws"></i></li>
            <li class="list-inline-item"><i class="fab fa-js-square"></i></li>
            <li class="list-inline-item"><i class="fab brands fa-php"></i></li>
            <li class="list-inline-item"><i class="fab fa-wordpress"></i></li>
            <li class="list-inline-item"><i class="fab fa-python"></i></li>
            <li class="list-inline-item"><i class="fab brands fa-bootstrap"></i></li>
            <li class="list-inline-item"><i class="fab brands fa-html5"></i></li>
            <li class="list-inline-item"><i class="fab brands fa-css3"></i></li>
            <li class="list-inline-item"><i class="fab brands fa-git"></i></li>
            <li class="list-inline-item"><i class="fab brands fa-trello"></i></li>
        </ul>
        <br>
        <div class="subheading mb-3">Workflow</div>
        <ul class="fa-ul mb-0">
            <li>
                <span class="fa-li"><i class="fas fa-check"></i></span>
                Rest API using Golang Echo and Gorm
            </li>
            <li>
                <span class="fa-li"><i class="fas fa-check"></i></span>
                Unit Testing using Mockery
            </li>
            <li>
                <span class="fa-li"><i class="fas fa-check"></i></span>
                Deployment to Docker and AWS
            </li>
            <li>
                <span class="fa-li"><i class="fas fa-check"></i></span>
                Agile Development & Scrum
            </li>
        </ul>
    </div>
</section>
